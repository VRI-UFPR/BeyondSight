...
def sample_cube_points(center,radius,step_size):
    start = np.array(center) - radius
    end = np.array(center) + radius

    # xv = np.mgrid[start[0]:end[0]+step_size:step_size, start[1]:end[1]+step_size:step_size, start[2]:end[2]+step_size:step_size].reshape(3,-1).T
    #no need for step on height y axis
    xv = np.mgrid[start[0]:end[0]+step_size:step_size, start[1]:start[1]+step_size:step_size, start[2]:end[2]+step_size:step_size].reshape(3,-1).T
    return xv
        ...
        for class_name in scenes[scene_name]:
            ...
            for current_episode_id in range(n_episodes):
                ...
                for retry in range(env.number_retries_per_target):
                    ...
                    if (smallest_dist_euclid<1)or(smallest_dist_euclid>20):
                        continue
                    ...
                    if((smallest_dist<1)or(smallest_dist>20)):
                        continue
            ...
            if(episodes):
                ...
                dst_folder = "/habitat-challenge-data/mine_objectgoal_mp3d/single_scene_all_classes_train4/content/"
                ...
                dst_folder = "/habitat-challenge-data/mine_objectgoal_mp3d/single_scene_all_classes_test4/content/"
                filename = dst_folder+scene_name+".json.gz"
