...
def generate_dataset(extractor, mode, path):
    ...
    print("image_id",image_id,"ann_id",ann_id,flush=True)
    ...
    if os.path.isfile(filename):
        ...
        tmp_dict['images'] = tmp_dict['images']+images
        tmp_dict['annotations'] = tmp_dict['annotations']+annotations
...
path = "/habitat-challenge-data/mp3d_2021_coco_style/"+str(split)+"/"


with open(filename, 'r') as f:
    tmp_dict = json.load(f)

SCENE_FILEPATHS=tmp_dict[str(split)]
del tmp_dict
