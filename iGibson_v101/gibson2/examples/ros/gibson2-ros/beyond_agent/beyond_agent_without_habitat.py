...
'''
class BeyondAgent():
    # def __init__(self, device, config, batch_size, out_is_action=False):
    def __init__(self, device, config, batch_size, out_is_action=True):

        self.out_is_action = out_is_action
        ...
        config.MODEL_PATH = config.BEYOND.PLANNER.CHECKPOINT
        ...
        with torch.no_grad():
            ...
            for i in range(self.batch_size):
                ...
                if(euclidean_distance<self.config.TASK_CONFIG.TASK.SUCCESS_DISTANCE):
                    ...
                    # self.update_followers_goal(i,_mean.tolist())
            ######################################################
            #avoid memory leak with ros
            if(len(self.array_of_preds[env_idx])>500):
            # if(len(self.array_of_preds[env_idx])>8):
                self.array_of_preds[env_idx]=[]
            ###############################################
            ...
            if(self.out_is_action==True):
                '''
                    If predicting discrete actions
                '''
                ...
                '''
                    Polar point goal
                '''
                return observations['pointgoal_with_gps_compass']
    ###############################################
