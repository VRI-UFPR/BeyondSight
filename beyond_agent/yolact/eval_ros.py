...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    target_fps   = round(30)
    frame_width  = round(640)
    frame_height = round(480)
    ...
    def get_next_frame():
        ...
        if len(buffer)>0:
            ...
            # frames = np.zeros((15,480,640,3), dtype=np.uint8)
            # frames = np.zeros((1,480,640,3), dtype=np.uint8)
            frames = np.zeros((1,240,320,3), dtype=np.uint8)

        for idx in range(args.video_multiframe-1):
            if len(buffer)>0:
                frame = buffer.pop(0)
                frames.append(frame)
            else:
    ...
    first_batch = eval_network(transform_frame(get_next_frame()))
    print('Done.')
    ...
    # rospy.Subscriber("mymsgs", String,
    #                  get_next_frame,  queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        if not vid_done:
            next_frames = pool.apply_async(get_next_frame)
        else:
            next_frames = None
        ...
        #
        # for frame in active_frames:
        #     cv2.imshow('output', prep_frame(frame['value'],'30.0'))
        #     cv2.waitKey(1)
        ...
        # cv2.imshow(path, frame['value'])



        # ler me_buffer.qsize() > 100:
        #     time.sleep(0.001)
        #
        # start_time = time.time()
        #
...
#     return bridge.cv2_to_imgmsg(rgb, encoding="rgb8")
