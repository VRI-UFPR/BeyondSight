...
def step(datum, epoch, iteration, optimizer, net, loss_avgs, time_avg, epoch_size, last_time,step_index,yolact_net,save_path):
    global loss_types # Forms the print order

    # # Stop if we've reached an epoch if we're resuming from start_iter
    # if iteration == (epoch+1)*epoch_size:
    #     return last_time,step_index,iteration,net,optimizer,loss_avgs,time_avg,yolact_net
    #
...
def train():
    ...
    #manually shuffle the ids to allow rebuild dataset and continue idxs
    save_path = lambda epoch, iteration: SavePath(cfg.name, epoch, iteration).get_path(root=args.save_folder)

    # for epoch in range(num_epochs):
    #     # Resume from start_iter
    #     if not ((epoch+1)*epoch_size < iteration):
    #         break
    # print("starting from epoch",epoch)
    # dataset.internal_shuffle(epoch)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            ####################################################################
            if(initial_iter - iteration != 0):
                '''
                end of epoch stop to avoid mem leak
                '''
                print('Stopping early. Saving network...')
                ...
                exit(3)
            ####################################################################
            # dataset.internal_shuffle(epoch+1)
